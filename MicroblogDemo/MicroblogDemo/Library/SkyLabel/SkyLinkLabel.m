...
- (void)setupTextSystem
{
    ...
    _linkDetectionTypes = SkyLinkDetectionTypeAll;
    ...
    //默认的回调
    self.linkTapHandler = ^(SkyLinkType linkType, NSString *string, NSRange range) {
        NSLog(@"Link Tap Handler");
    };
    self.linkLongPressHandler = ^(SkyLinkType linkType, NSString *string, NSRange range){
...
- (void)setLinkDetectionTypes:(SkyLinkDetectionTypes)linkDetectionTypes
{
...
- (void)updateTextStoreWithAttributedString:(NSAttributedString *)attributedString
{
    if (attributedString.length != 0){
        attributedString = [SkyLinkLabel sanitizeAttributedString:attributedString];
    }
...
- (NSAttributedString *)addLinkAttributesToAttributedString:(NSAttributedString *)string linkRanges:(NSArray *)linkRanges
{
    ...
    for (NSDictionary *dictionary in linkRanges)
    {
        ...
        //链接地址
        //        if ((SkyLinkType)[dictionary[@"linkType"] intValue] == SkyLinkTypeURL)
        //        {
...
- (NSArray *)getRangesForLinks:(NSAttributedString *)text
{
    ...
    //用户昵称
    if (self.linkDetectionTypes & SkyLinkDetectionTypeUserHandle)
    {
        [rangesForLinks addObjectsFromArray:[self getRangesForUserHandles:text.string]];
    }
    //内容标签
    if (self.linkDetectionTypes & SkyLinkDetectionTypeHashTag)
    {
        [rangesForLinks addObjectsFromArray:[self getRangesForHashTags:text.string]];
    }
    //链接地址
    if (self.linkDetectionTypes & SkyLinkDetectionTypeURL)
    {
        [rangesForLinks addObjectsFromArray:[self getRangesForURLs:self.attributedText]];
    }
    //电话号码
    if (self.linkDetectionTypes & SkyLinkDetectionTypePhoneNumber)
    {
        [rangesForLinks addObjectsFromArray:[self getRangesForPhoneNumbers:text.string]];
    }
    //主题内容
    if (self.linkDetectionTypes & SkyLinkDetectionTypeTheme)
    {
        [rangesForLinks addObjectsFromArray:[self getRangesForThemeTags:text.string]];
    }
    //......
...
- (NSArray *)getRangesForUserHandles:(NSString *)text
{
    ...
    for (NSTextCheckingResult *match in matches)
    {
        ...
        [rangesForUserHandles addObject:@{
                                          @"linkType" : @(SkyLinkTypeUserHandle),
 ...
 * 所有话题内容
 */
- (NSArray *)getRangesForThemeTags:(NSString *)text
{
    NSMutableArray *rangesForThemeTags = [[NSMutableArray alloc] init];
    NSError *error = nil;
    ...
    for (NSTextCheckingResult *match in matches)
    {
        ...
        [rangesForThemeTags addObject:@{
                                       @"linkType" : @(SkyLinkTypeTheme),
                                       @"range"    : [NSValue valueWithRange:matchRange],
                                       @"link"     : matchString
                                       }];
    }
    return rangesForThemeTags;
    ...
    for (NSTextCheckingResult *match in matches)
    {
        ...
        if ([match resultType] == NSTextCheckingTypePhoneNumber){
            [rangesForPhoneNumbers addObject:@{
                                               @"linkType" : @(SkyLinkTypePhoneNumber),
                                               @"range"    : [NSValue valueWithRange:matchRange],
                                               @"link"     : matchString
                                               }];
        }
...
-(BOOL)layoutManager:(NSLayoutManager *)layoutManager shouldBreakLineByWordBeforeCharacterAtIndex:(NSUInteger)charIndex
{
    ...
    {
        NSRange range = [[dictionary objectForKey:@"range"] rangeValue];
        SkyLinkType linkType = [[dictionary objectForKey:@"linkType"] integerValue];
        if (linkType == SkyLinkTypeURL) {
...
- (void)longPressLabel:(UILongPressGestureRecognizer *)recognizer
{
    ...
    if (link) {
        ...
        SkyLinkType linkType = (SkyLinkType)[[link objectForKey:@"linkType"] intValue];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touchedLink){
        ...
        SkyLinkType linkType = (SkyLinkType)[[touchedLink objectForKey:@"linkType"] intValue];
